...
import { buildWorkerTemplate } from "./inject.js";

const testS3Prefix = "s3://frontend/deploy";

const genHtml = (input) => `<html lang="en"><head></head><body>${input}</body></html>`;
...
test("converts to liquid", () => {
  const testCases = [
    ...
    {
      description: "input with s3-file class and literal s3 file gets literal liquid tag",
    ...
    {
      description: "input with s3-file class and variable input gets variable liquid tag",
